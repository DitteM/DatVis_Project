  ...
  output$beskrivelse_1 <- renderPlotly({
    ...
    if(input$order_input == "Alfabetisk"){
      p <- data_work %>%
        ...
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~beskrivelse_1, type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_1",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
                             '</br> Varegruppe: ', beskrivelse_1,
                             '</br> V\u00E6rdi: ', round(.data[[y_in]],2))) %>%
      ...
      p <- data_work %>%
        filter(is.na(beskrivelse_2)) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_1, .data[[y_in]]), type = "bar",
                color = ~I(pos),
                source = "beskrivelse_1",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
      ...
      p <- data_work %>%
        filter(is.na(beskrivelse_2)) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_1, desc(.data[[y_in]])), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_1",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
  ...
  output$beskrivelse_2 <- renderPlotly({
    ...
    if(input$order_input == "Alfabetisk"){
      p <- data_work %>% 
        ...
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~beskrivelse_2, type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_2",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
                             '</br> Varegruppe: ', beskrivelse_1,
                             '</br> V\u00E6rdi: ', round(.data[[y_in]],2))) %>%
      ...
      p <- data_work %>% 
        ...
        filter(beskrivelse_1 %in% beskrivelse_1()) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_2, .data[[y_in]]), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_2",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
      ...
      p <- data_work %>% 
        ...
        filter(beskrivelse_1 %in% beskrivelse_1()) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_2, desc(.data[[y_in]])), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_2",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
  ...
  output$beskrivelse_3 <- renderPlotly({
    ...
    if(input$order_input == "Alfabetisk"){
      p <- data_work %>% 
        ...
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~beskrivelse_3, type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_3",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
                             '</br> Varegruppe: ', beskrivelse_1,
                             '</br> V\u00E6rdi: ', round(.data[[y_in]],2))) %>%
      ...
      p <- data_work %>% 
        ...
        filter(beskrivelse_2 %in% beskrivelse_2()) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_3, .data[[y_in]]), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_3",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
      ...
      p <- data_work %>% 
        ...
        filter(beskrivelse_2 %in% beskrivelse_2()) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_3, desc(.data[[y_in]])), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_3",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
  ...
  output$beskrivelse_4 <- renderPlotly({
    ...
    if(input$order_input == "Alfabetisk"){
      p <- data_work %>% 
        ...
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~beskrivelse_4, type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_4",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
                             '</br> Varegruppe: ', beskrivelse_1,
                             '</br> V\u00E6rdi: ', round(.data[[y_in]],2))) %>%
      ...
      p <- data_work %>% 
        filter(beskrivelse_3 %in% beskrivelse_3()) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_4, .data[[y_in]]), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_4",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
      ...
      p <- data_work %>% 
        filter(beskrivelse_3 %in% beskrivelse_3()) %>%
        mutate(pos = ifelse(.data[[y_in]] >= 0, "#00008b", "#e5de00")) %>%
        plot_ly(x = ~.data[[y_in]], y = ~reorder(beskrivelse_4, desc(.data[[y_in]])), type = "bar", 
                color = ~I(pos),
                source = "beskrivelse_4",
                hoverinfo='text',
                text= ~paste('</br> \u00C5r-m\u00E5ned: ', format(Dato,"%Y-%m"),
